...
test("Copy and destroy test fixture 1", () => {
	...
	expect(fs.readFileSync(join(fixture.dir, "test-file.txt")).toString()).toBe(
		"Test information\n"
	);
});
...
test("Load test fixture 2", () => {
	...
	expect(Object.prototype.hasOwnProperty.call(fixture.obj, "testBool")).toBe(
		true
	);
...
test(`Try to use read() on a file that doesn't exist`, () => {
	...
	try {
		...
		expect(err.message).toBe(
			`Invalid file in fixture read: ${join(fixture.dir, filename)}`
		);
	}
});
